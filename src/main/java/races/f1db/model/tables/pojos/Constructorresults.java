/*
 * This file is generated by jOOQ.
 */
package races.f1db.model.tables.pojos;


import java.io.Serializable;

import javax.annotation.Generated;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.11.7"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Constructorresults implements Serializable {

    private static final long serialVersionUID = 917675162;

    private Integer constructorresultsid;
    private Integer raceid;
    private Integer constructorid;
    private Double  points;
    private String  status;

    public Constructorresults() {}

    public Constructorresults(Constructorresults value) {
        this.constructorresultsid = value.constructorresultsid;
        this.raceid = value.raceid;
        this.constructorid = value.constructorid;
        this.points = value.points;
        this.status = value.status;
    }

    public Constructorresults(
        Integer constructorresultsid,
        Integer raceid,
        Integer constructorid,
        Double  points,
        String  status
    ) {
        this.constructorresultsid = constructorresultsid;
        this.raceid = raceid;
        this.constructorid = constructorid;
        this.points = points;
        this.status = status;
    }

    public Integer getConstructorresultsid() {
        return this.constructorresultsid;
    }

    public void setConstructorresultsid(Integer constructorresultsid) {
        this.constructorresultsid = constructorresultsid;
    }

    public Integer getRaceid() {
        return this.raceid;
    }

    public void setRaceid(Integer raceid) {
        this.raceid = raceid;
    }

    public Integer getConstructorid() {
        return this.constructorid;
    }

    public void setConstructorid(Integer constructorid) {
        this.constructorid = constructorid;
    }

    public Double getPoints() {
        return this.points;
    }

    public void setPoints(Double points) {
        this.points = points;
    }

    public String getStatus() {
        return this.status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder("Constructorresults (");

        sb.append(constructorresultsid);
        sb.append(", ").append(raceid);
        sb.append(", ").append(constructorid);
        sb.append(", ").append(points);
        sb.append(", ").append(status);

        sb.append(")");
        return sb.toString();
    }
}
